...
export default function Cart() {
  // Mock initial cart state using the first two products
                ...
                {cartItems.map(item => (
                  <div key={item.id} className="p-6 grid grid-cols-1 sm:grid-cols-12 gap-6 items-center">
                    ...
                    <div className="sm:col-span-6 flex gap-4 items-center">
                      ...
                      <div>
                        <Link to={`/shop/${item.slug}`} className="font-display text-lg text-[#101726] hover:text-[#b38947] transition-colors line-clamp-1">
                          {item.name}
                        </Link>
                        <p className="text-gray-500 text-sm mt-1">${item.price.toFixed(2)}</p>
                        <button 
                          onClick={() => removeItem(item.id)}
                          className="text-red-400 hover:text-red-600 text-sm flex items-center gap-1 mt-3 transition-colors"
                        >
                    ...
                    <div className="sm:col-span-3 text-right">
                      <span className="font-semibold text-lg text-[#101726]">
          ...
          <div className="w-full lg:w-96 shrink-0">
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sticky top-28">
              ...
              <div className="flex justify-between items-end mb-8">
                ...
                <span className="font-display text-3xl font-bold text-[#b38947]">
                  ${total.toFixed(2)}
                </span>
              </div>
